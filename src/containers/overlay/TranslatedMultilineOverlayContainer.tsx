...
interface IMultilineOverlayProviderContainerProps {
  ...
  caption?: store.ITranslatedMultilineCaption;
  defaultStyle?: store.ICaptionsStyle;
}

function TranslatedMultilineOverlayContainer(props: IMultilineOverlayProviderContainerProps) {
  const { player } = React.useContext(PlayerContext);
  const logger = React.useMemo(
    () => ReactLoggerFactoryHelper.build('MultilineOverlayContainer'), []
  );
  ...
  >(state => state.present.translatedMultiline.defaultLocation);
  ...
  const backgroundColor = line?.style?.background ?? props.defaultStyle?.background;
  ...
  const lineStyle = React.useMemo(() => {
    ...
    let background = StyleParser.backgroundParse(backgroundColor, selectedPreviewType);
    if (background) {
      style.background = rgbaToString(background);
    }

    return style;
  }, [props.fontUnitSize, backgroundColor, selectedPreviewType]);
  ...
  focusMeta = (
    ...
    && focusMeta.type === "translatedMultiline"
  ...
  function _handleLocationChangeEnd(location: IPoint, meta?: ICaptionEventMeta) {
    ...
    const captions = store.default.getState().present.translatedMultiline.captions;
    ...
    TranslatedMultilineActions.updateCaptionById(paragraph);
  }
...
export default TranslatedMultilineOverlayContainer;